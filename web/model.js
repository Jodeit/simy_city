...
window.SIMY_MODEL = {
  ...
  "land_uses": {
    ...
    "dollar_store": {
      "label": "Dollar Store (Dollar General/Family Dollar-type)",
      "requires": {
        "demand": {
          "min_households_drive_time": 4000,
          "drive_time_min": 12
      ...
      "notes": "A rooftop-headcount demand read at a wide-but-thin trade area (8 km), min_households_drive_time (4000, lower than convenience_store's 6000 and grocery_store's 12000 \u2014 the whole point of a dollar store is working off a thinner, more spread-out rooftop count than either), min_buildable_acres (0.9, in convenience_store/car_wash's range), and min_distance_km_from_nearest (1.5, tighter than grocery_store's 2.0 \u2014 dollar-store chains cluster closer together than full grocers) are first-draft heuristics \u2014 refine freely, same as every other threshold here. No `transportation` AADT gate, unlike convenience_store \u2014 a dollar store's draw is short-drive/errand convenience, not visibility from a passing arterial. Wired up via `standardUseVerdict` directly (same demand+site-size+competitor-distance shape grocery_store/ home_improvement_store/fitness_center/vet_clinic already established), no new gate-shape work needed. OSM tags this with two live tags \u2014 `shop=variety_store` primarily, with `shop=discount` as a secondary/ fallback tag OR'd into the same live competitor/site query \u2014 same messy-tagging caveat urgent_care/child_care_center/car_wash/pharmacy/ convenience_store/home_improvement_store/bank_branch already documented.\n"
    }
  },
  "actor_uses": {
    "employment_center": {
