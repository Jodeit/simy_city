...
window.SIMY_MODEL = {
  ...
  "land_uses": {
    ...
    "solar_farm": {
      ...
      "notes": "Inverts every demand-driven use above: a solar farm wants cheap, flat, mostly-vacant acreage with a nearby grid interconnection, and doesn't care how many rooftops are nearby at all. Verdict (`maybeRenderSolarVerdict`, web/explore.html) is two-gate, not the usual three \u2014 acreage + substation distance, no demand leg to wait on \u2014 the same `demandKind:\"trivial\"` shape data_center's `BEST_FIT_USES` entry already established, reusing data_center's own compQ-as-power- query trick (no separate `powerQ` fetch needed). This item was drafted expecting to ship with reverse search disabled (\"no ranking signal, same as data_center/residential_subdivision today\") \u2014 that assumption was already stale: `reverseSearchSignals` (web/logic.js) gained `preferNearComp`/`preferFarDemand` since then, and both existing holdouts already resolve a real signal from it. Solar farm is no exception \u2014 `requires.power.prefer_substation_within_km` alone gives it a correct `preferNearComp` signal (closer to a substation is genuinely better) with no wrong-shaped-signal risk, so the search button ships enabled here too.\n"
    }
  },
  "actor_uses": {
    "employment_center": {
