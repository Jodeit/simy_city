...
window.SIMY_MODEL = {
  ...
  "land_uses": {
    ...
    "truck_stop": {
      ...
      "induces": {
        "transportation": {
          "note": "high daily truck-trip generation at the interchange ramp; may require a dedicated deceleration/acceleration lane"
        }
      },
      "impacts": {
        "habitat": "low",
      ...
      "notes": "The second no-`requires.demand` land use (after solar_farm): a truck stop's viability has nothing to do with nearby households, only interstate-interchange access, so `USE_DEMAND.truck_stop.roofNeed` is 0 (display-only rooftop count, never gated) same as solar_farm/ data_center. Verdict (`maybeRenderTSVerdict`, web/explore.html) is a two-gate PASS/SHORT mirroring solar_farm's shape exactly, just swapping substation-distance for the highway-AADT leg every warehouse_club-style use already fetches (`arcgisNearQuery(AADT_SOURCE,...)`, reused here at a tight 1.5 km radius rather than warehouse_club's wide trade-area one \u2014 trucks need a ramp right there, not \"somewhere in the same zip code\"). `reverseSearchSignals` (web/logic.js) needs no changes: with no `competition`/`power.prefer_substation_within_km`/`demand_signals` block, none of its four signals turn on, so `updateSearchAvailability` already disables the reverse-search toggle for this use automatically \u2014 the right outcome (a road-network-proximity read doesn't fit `rankCandidates`' point-competitor/demand-radius shapes) without bolting on a wrong-shaped ranking signal.\n"
    }
  },
  "actor_uses": {
    "employment_center": {
