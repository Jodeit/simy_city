...
/* ---- global keyboard-shortcut guard ----
   Pure predicate for "should a document-level keydown listener ignore this
   event because the user is typing somewhere?" — factored out of the inline
   check wireHelp()'s `?` listener used to duplicate, now shared by every
   global shortcut (help, pin, compare) so they all skip the same set of
   focusable text targets rather than drifting out of sync. Takes the event
   target directly (not the event) so a test can pass a plain {tagName}/
   {isContentEditable} object without constructing a real KeyboardEvent. */
...
/* ---- Web Share API payloads (native OS share sheet) ----
   Pure builders so the feature-detect branch (does this browser have
   navigator.share?) and the payload shape can both be unit tested without
   a real navigator.share, which requires a live user gesture and never
   resolves in a synthetic/headless test. `hasWebShare(nav)` takes the
   navigator object as an argument (rather than reading the global) so a
   test can pass `{}`/`{share(){}}` fakes; explore.html calls it with the
   real `navigator`. */
function hasWebShare(nav){
  return !!(nav && typeof nav.share==="function");
}
function sharePayloadForLink(link,label){
  return {title:label?`SIMyCity: ${label}`:"SIMyCity",url:link};
}
function sharePayloadForCase(text,label){
  return {title:`SIMyCity: ${label} — should we build it here?`,text};
}
...
/* ---- CSV-file bulk import: pick the address out of a possibly-tabular row ----
   The bulk-import textarea only ever held one address (or "lat, lng" pair)
   per line. A user-picked .csv/.txt file can hold that same one-value-a-line
   shape (a plain address list, or an actual spreadsheet export with separate
   Address/City/State/Zip columns) — `extractAddressColumn` runs per line
   before the text is handed to `parseBulkAddressList`, so both still funnel
   through the exact same paste pipeline. A "lat, lng" pair is recognized
   first and passed through whole (`parseCoordPair`, same as everywhere
   else). Otherwise the line is split RFC-4180-style (a doubled `""` inside
   a quoted field is a literal quote, not a field break, so a quoted column
   with an embedded comma — e.g. `"123 Main St, Austin, TX",78701` — stays
   one field). A quoted field anywhere is treated as a deliberate CSV export
   and only the first column survives. Without quoting, up to 3
   comma-separated segments is treated as a plain "street, city, state"
   address and kept whole — real spreadsheet exports (Address, City, State,
   Zip, County, …) tend to run 4+ columns; only at that width, or when
   quoting signals real CSV, is the line actually split and the rest of the
   row dropped rather than failing the whole file. This is a heuristic. An
   unquoted 3-segment CSV row that happens to look like an address will be
   kept whole (geocoded as-is, which usually still resolves); a genuinely
   4+ column row that happens to be one long unquoted address will lose its
   tail. Pasting directly into the textarea is unaffected either way — this
   function only runs on file-derived lines. */
function splitCsvFields(line){
  const s=String(line||"");
  const fields=[];
  let cur="", inQuotes=false, quoted=false;
...
function extractAddressColumn(line){
  ...
  if(!s)return "";
  if(parseCoordPair(s))return s;
  const {fields,quoted}=splitCsvFields(s);
  if(fields.length<=1)return fields[0].trim();
  if(!quoted && fields.length<=3)return s;
...
/* ---- highlighting the best value per Compare-table numeric row ----
   Given the same pins array renderCompare() sorts and a numeric key
   (acres/value), returns the index/indices of the max value — an array
   since a tie should highlight every winner, not arbitrarily pick one.
   Ignores null/undefined values entirely (a parcel missing the field isn't
   a contender), so an all-null column returns []. */
function bestValueIndices(pins,key){
  const list=pins||[];
  let max=null;
  list.forEach(p=>{
  ...
  if(max==null) return [];
  const out=[];
  list.forEach((p,i)=>{ if(p&&p[key]===max) out.push(i); });
  return out;
}
...
/* Nearest airfield to a point, from a raw Overpass response — backs the
   developer-checklist's informational "nearest airport" row (not a verdict
   input for any land use, just due-diligence context: noise/height/FAA-
   obstruction questions matter well beyond the ~1 km transit-proximity
   radius, hence the wider query radius callers use). Mirrors
   `nearestTransitStop`'s element-shape handling exactly (node lat/lon or
   way/relation `center`, malformed/missing-coordinate elements dropped) —
   only the labeling differs: name tag, else IATA/ICAO code in parens, else
   a plain "unnamed airfield" fallback. */
function nearestAirport(json,center){
  if(!center)return null;
  let best=null;
  ((json&&json.elements)||[]).forEach(e=>{
    if(!e)return;
    ...
    if(best&&km>=best.km)return;
    const tags=e.tags||{};
    const code=tags.iata?`IATA: ${tags.iata}`:tags.icao?`ICAO: ${tags.icao}`:null;
    const label=tags.name?(code?`${tags.name} (${code})`:tags.name)
      :code?`unnamed airfield (${code})`
      :"unnamed airfield";
    best={lat,lng,km,label};
  });
  return best;
}
...
// "Best fit here" ranked-table CSV/PDF export — the third and last of the
// three multi-result views (reverse-search candidates, Compare list, and
// this one) to get an export, same "row-shape function + existing exporter"
// pattern candidatesToCsvRows/buildCandidatesReportText and
// buildCompareReportText already established. `e` is one rankLandUseVerdicts()
// entry: standardUseVerdict's pass/demandOk/siteOk/farOk/aadtOk/subOk/mudOk/
// ratio fields merged with the per-use thresholds/reads runBestFit already
// carries (acres, minAcres, nearestComp, minCompetitorKm, aadtHit, minAadt,
// subKm, maxSubstationKm, mud, requireDistrict) — the exact shape
// web/explore.html's `scored` array already builds for on-screen rendering,
// reused as-is rather than re-deriving anything. `unit` (default "ac") drives
// the same display-only area-unit conversion formatArea uses everywhere else.
function bestFitReasonText(e,unit){
  unit=unit||"ac";
  const fmt=a=>a==null?"—":(formatArea(a,unit)??"—");
  const parts=[
    e.demandOk?`✓ demand ${Math.round(e.ratio*100)}% of need`
      : `✗ demand only ${Math.round(e.ratio*100)}% of need`,
  ];
  if(e.minAcres!=null)parts.push(
    e.siteOk?`✓ site ${fmt(e.acres)}`
      : e.acres==null?"? site size unlisted"
      : `✗ site only ${fmt(e.acres)} (need ≥${fmt(e.minAcres)})`
  );
...
// `labels` maps each entry's `key` to its display label — callers already
// have this from MODEL.land_uses/actor_uses, kept out of this dependency-free
// module rather than reaching for a global MODEL here. A key missing from
// `labels` falls back to the raw key rather than throwing or showing "undefined".
function bestFitToCsvRows(ranked,labels,unit){
  labels=labels||{};
  const rows=[["#","Use","Verdict","Demand (% of need)","Reason"]];
  (ranked||[]).forEach((e,i)=>rows.push([
    i+1, labels[e.key]||e.key, e.pass?"PASS":"SHORT",
    e.ratio!=null?Math.round(e.ratio*100):"",
    bestFitReasonText(e,unit),
  ]));
  return rows;
}
// Mirrors buildCandidatesReportText/buildCompareReportText's role for the
// third multi-result view's printable PDF report. `noRead` (use keys with no
// shared demand/site/competitor read, or no rooftop data at this point) is
// listed separately rather than silently dropped, same as the on-screen
// "Not scored here" line renderBestFit already shows.
function buildBestFitReportText(center,ranked,noRead,labels,unit){
  ranked=ranked||[]; noRead=noRead||[]; labels=labels||{};
  const lat=(center&&typeof center.lat==="number")?center.lat.toFixed(4):"?";
  const lng=(center&&typeof center.lng==="number")?center.lng.toFixed(4):"?";
  let t=`SIMyCity — "Best fit here" ranked results\n`;
  t+=`Point: ${lat}, ${lng}\n`;
  t+=`${ranked.length} use${ranked.length===1?"":"s"} scored\n\n`;
  ranked.forEach((e,i)=>{
    t+=`  ${i+1}. ${labels[e.key]||e.key} — ${e.pass?"PASS":"SHORT"}\n`;
    t+=`     ${bestFitReasonText(e,unit)}\n`;
  });
  if(noRead.length)t+=`\nNot scored here: ${noRead.map(k=>labels[k]||k).join(", ")}\n`;
  t+=`\nBuilt on open public data · github.com/jodeit/simy_city\n`;
  return t;
}
