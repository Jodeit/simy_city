...
/* ---- "Best fit here" CSV/PDF export ----
   The Compare-parcels modal and the reverse-search candidate list both
   already export CSV + PDF (candidatesToCsvRows/buildCandidatesReportText,
   compareToCsvRows/buildCompareReportText above) — this is the third and
   last multi-row result view, so it gets the same treatment: reuse
   toCsv/buildSimplePdf, just fed a rankLandUseVerdicts() result list
   instead. bestFitReasonText moved here (was explore.html-only) so both the
   CSV/PDF row-shape and the live panel render off one implementation — it
   takes `unit` as a plain argument (formatArea's display unit) rather than
   reading explore.html's `areaUnit` global, same "pure function, caller
   supplies context" contract as buildCompareReportText's own `unit` param. */
function bestFitReasonText(e,unit){
  const fa=a=>formatArea(a,unit)??"—";
  const parts=[
    e.demandOk?`✓ demand ${Math.round(e.ratio*100)}% of need`
      : `✗ demand only ${Math.round(e.ratio*100)}% of need`,
  ];
  if(e.minAcres!=null)parts.push(
    e.siteOk?`✓ site ${fa(e.acres)}`
      : e.acres==null?"? site size unlisted"
      : `✗ site only ${fa(e.acres)} (need ≥${fa(e.minAcres)})`
  );
...
// Shapes a rankLandUseVerdicts() result list into toCsv()-ready rows for the
// "Best fit here" ranked table (rank, use, PASS/SHORT, demand ratio, and the
// same per-gate reason text the panel renders — the ✗-marked parts of which
// are exactly "the specific gate(s) that failed") — same "reuse the existing
// CSV plumbing" pattern the Compare/candidates CSV exports already
// established. `unit` drives the same display-only acreage conversion
// bestFitReasonText/formatArea use everywhere else.
function bestFitToCsvRows(ranked,unit){
  const rows=[["#","Use","Result","Demand ratio","Reason"]];
  (ranked||[]).forEach((e,i)=>rows.push([
    i+1, e.label||e.key, e.pass?"PASS":"SHORT",
    e.ratio!=null?Math.round(e.ratio*100)+"%":"",
    bestFitReasonText(e,unit),
  ]));
  return rows;
}
// Builds the text for a printable/exportable report summarizing the
// "🏆 Best fit here" ranked table (every scored use, PASS/SHORT, and its
// per-gate reason text) — mirrors buildCandidatesReportText's role for the
// reverse-search PDF, just built from one point's rankLandUseVerdicts()
// result (many uses) instead of one use's ranked candidate points.
// `notRankedLabels` are the uses with no shared demand/site/competitor read
// at all here (already-resolved display labels, same set renderBestFit's own
// "Not scored here" line lists) — reported separately since they have no
// PASS/SHORT verdict. A missing/malformed center falls back to "?, ?" rather
// than throwing or emitting "NaN, NaN", same contract as
// buildCandidatesReportText.
function buildBestFitReportText(center,ranked,notRankedLabels,unit){
  ranked=ranked||[]; notRankedLabels=notRankedLabels||[];
  const lat=(center&&typeof center.lat==="number")?center.lat.toFixed(4):"?";
  const lng=(center&&typeof center.lng==="number")?center.lng.toFixed(4):"?";
  let t=`SIMyCity — best fit at this point\n`;
  t+=`Point: ${lat}, ${lng}\n`;
  t+=`${ranked.length} use${ranked.length===1?"":"s"} scored\n\n`;
  ranked.forEach((e,i)=>{
    t+=`  ${i+1}. ${e.label||e.key} — ${e.pass?"PASS":"SHORT"}\n`;
    t+=`     ${bestFitReasonText(e,unit)}\n`;
  });
  if(notRankedLabels.length)t+=`\nNot scored here: ${notRankedLabels.join(", ")}\n`;
  t+=`\nBuilt on open public data · github.com/jodeit/simy_city\n`;
  return t;
}
